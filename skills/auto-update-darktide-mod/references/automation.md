...
- Every pinned resume validates the complete installed Skill package against the fixed run-local `review-artifacts/skill-source-pin.json` before acquiring the reservation worker or state writer lease. A package file, content hash, commit, packaged reference, or materialized reference mismatch fails as package drift without changing state, lock evidence, source, or the immutable pin. The completed-stage fast path repeats this binding check immediately before receipt reuse.
