...
- Before rebuilding checkpoints, the runner inspects the run's unique remote-tracking branch. If mutable state incorrectly says `published=false` after that branch exists, it atomically repairs `published=true` and stops before checkpoint mutation. Continue only with the Workflow's append-only same-run refresh procedure; never edit the flag back to false.
