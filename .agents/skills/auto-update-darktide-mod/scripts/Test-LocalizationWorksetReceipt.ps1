...
$before = Get-LuaLocalizationDocument -Bytes $newBytes -DisplayPath $newFull -SourceId ([string]$workset.sourceId)
...
if ($bindingValueCount -eq 3) {
    ...
    $oldDocument = Get-LuaLocalizationDocument -Bytes $oldBytes -DisplayPath ([string]$workset.old.path) -SourceId ([string]$workset.sourceId)
...
$expectedReceiptEdits = @($expectedEdits | ForEach-Object {
    ...
    if ($oldBytes.Length -gt 0) { [Array]::Copy($newBytes, [int64]$_.startByte, $oldBytes, 0, $oldBytes.Length) }
    [ordered]@{
