...
Version `0.3.13` canonicalizes raw-install normalization records to the unique tracked Git spelling when `core.ignorecase=true`. This preserves the exact index identity for archives whose physical directory casing differs from an existing tracked path, prevents C1 blob verification from addressing a path that Git normalized to another spelling, leaves untracked paths and case-sensitive repositories unchanged, and fails closed if tracked paths are ambiguous under ignore-case matching. Existing runs remain pinned to their recorded package without implicit migration.
