...
1. Commit the intended release snapshot, require a clean working tree and index, and run `pwsh -NoLogo -NoProfile -File ./scripts/Validate.ps1` before push. This is the only complete local gate and binds authority, integrity, formal static tools, repository/domain tests, and source-pin evidence to one unchanged commit. `scripts/Invoke-PrePushValidation.ps1` remains a thin wrapper for callers that need a pre-push entry point; repository, domain, and tool commands are component diagnostics rather than alternate release gates.
