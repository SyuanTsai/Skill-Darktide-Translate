...
Version `0.3.3` preserves the 0.3.2 state contract while completing same-run recovery hardening, clarifying the exact Schema 15 browser incoming path, and distinguishing valid Unix regular-file execute modes from overlapping Windows ZIP external attributes in both extraction and the independent Candidate Gate. Existing runs remain pinned to their recorded package and are never migrated implicitly.

Version `0.3.4` preserves existing state contracts while allowing bounded aggregate runs to stop at `localized`, resuming aggregate runs past idempotent stages after approved localization input, fixing repository-root workset finalization, and documenting the required Agent-prepared README/formal-hash handoff before evidence commits. Existing runs remain pinned to their recorded package and are never migrated implicitly.
